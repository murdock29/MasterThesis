...
class Logger(ExperimentLogger):
    ...
    def log_scalar(self, iter, name, value, group=None, curtime=None):
        ...
        # Raw dump
        entry = {"iter": iter, "name": name, "value": value, "group": group,
    ...
    def save_model(self, state_dict, task):
        curtime = datetime.now()
        torch.save(state_dict, os.path.join(self.exp_path, "models", "task{}.ckpt".format(curtime.strftime("%Y-%m-%d-%H-%M-%S"))))
