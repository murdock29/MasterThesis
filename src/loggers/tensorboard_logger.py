...
class Logger(ExperimentLogger):
    ...
    def __init__(self, log_path, exp_name, begin_time= None):
        ...
        self.new_path = os.path.join(self.exp_path, self.begin_time.strftime("%Y-%m-%d-%H-%M"))
        os.mkdir(self.new_path)
        self.tbwriter = SummaryWriter(self.new_path)


    def log_scalar(self, iter, name, value, group=None, curtime=None):
        self.tbwriter.add_scalar(tag="t{}/{}_".format(group, name),
                                 scalar_value=value,
                                 global_step=iter)
